...
for i in range(1, 20):
    ...
    print("Acurácia no conjunto de validação: ", accuracy_val, file=txt)
    print("Taxa de erro no conjunto de validação: ", taxa_de_erro_val, file=txt)
    ...
    taxa_de_erro_test = 1 - accuracy_test
    print("Acurácia no conjunto de teste: ", accuracy_test, file=txt)
    print("Taxa de erro no conjunto de teste: ", taxa_de_erro_test, file=txt)
    print("Matriz de Confusão\n ",confusion_matrix(y_test, opiniao_test), file=txt)
    TN, FP, FN, TP = confusion_matrix(y_test, opiniao_test).ravel()
    print("Sensibilidade: ",(TP/(TP+FN)), file=txt)
    print("Especificade: ",(TN/(FP+TN)), file=txt)
...
df = pd.DataFrame(accuracy_list, columns=['KNN', 'SVM', 'Decision Tree', 'Naive Bayes'])
